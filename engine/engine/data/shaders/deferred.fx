...
float getSpecular(float3 wPos, float3 L, float3 N, float3 ss_load_coords) {	
	...
	return LightingFuncGGX_OPT3(N, V, L, 1 - gloss.x, fresnel);
...
float4 PSLightShafts(VS_TEXTURED_OUTPUT input
	...
	float4 color = txDiffuse.Sample(samClampLinear, input.UV) * float4(Tint.xyz, 1);//float4(1, 0.5, 0.2, 1);
		...
		color.a *= txEmissive.Sample(samWrapLinear, input.wPos.xz + world_time.xx * 0.1).x;
	color.a *= txEmissive.Sample(samWrapLinear, input.wPos.yz - cos(world_time.xx) * 0.05).x;
	//color.a *= delta_z;
