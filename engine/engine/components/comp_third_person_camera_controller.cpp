...
void TCompThirdPersonCameraController::update(float elapsed) {
	...
	target_pos = XMVectorSetY(target_pos, prev_y);
	...
	//transform->position = XMVectorLerp(transform->position, target_pos, clamp(0.25f, 0, 1));
	//transform->rotation = XMQuaternionSlerp(transform->rotation, target_rot, clamp(0.25f, 0, 1));
