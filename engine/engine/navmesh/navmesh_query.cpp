  ...
  if (io.becomesPressed(CIOStatus::MOUSE_LEFT)) {
    // moving end point?
	  if (io.isPressed(CIOStatus::RUN)) {
		...
		if( data->m_navMesh )
			data->m_navQuery->findNearestPoly( 
			&pos,
			m_polyPickExt, 
			&m_filter,
			&m_endRef, 
...
void CNavmeshQuery::findPath(TPos& start, TPos& end) {
  ...
  if( start.set && end.set && m_startRef && m_endRef ) {
    ...
    if( m_npolys ) {
	  ...
	  data->m_navQuery->closestPointOnPoly(m_startRef, &start_pos_x, iterPos, 0);
	  data->m_navQuery->closestPointOnPoly(polys[npolys - 1], &end_pos_x, targetPos, 0);
...
void CNavmeshQuery::wallDistance( TPos& pos ) {
  ...
  if( pos.set && m_startRef ) {
	...
	data->m_navQuery->findDistanceToWall(m_startRef, &pos_x, 100.0f, &m_filter, &m_distanceToWall, m_hitPos, m_hitNormal);
  }
}
...
void CNavmeshQuery::raycast( TPos& start, TPos& end ) {
  ...
  if( start.set && end.set && m_startRef ) {
    ...
    m_npolys = 0;
    m_nstraightPath = 2;
	m_straightPath[0] = start_pos_x;
	m_straightPath[1] = start_pos_y;
	m_straightPath[2] = start_pos_z;
	data->m_navQuery->raycast(m_startRef, &start_pos_x, &end_pos_x, &m_filter, &t, m_hitNormal, m_polys, &m_npolys, MAX_POLYS);
    if( t > 1 ) {
      // No hit
		dtVcopy(m_hitPos, &end_pos_x);
      m_hitResult = false;
    }
    else {
      // Hit
		dtVlerp(m_hitPos, &start_pos_x, &end_pos_x, t);
      m_hitResult = true;
    }
    // Adjust height.
    if( m_npolys > 0 ) {
