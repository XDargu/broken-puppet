  ...
  for(iteratorAnimationAction = m_listAnimationAction.begin(); iteratorAnimationAction != m_listAnimationAction.end(); ++iteratorAnimationAction)
  {
    ...
    for(iteratorCoreTrack = listCoreTrack.begin(); iteratorCoreTrack != listCoreTrack.end(); ++iteratorCoreTrack)
    {
      ...
      if (bone_id == logic_delta_bone_id ) {
        CalVector logic_curr_translation = translation;
        
        // bone id must be a root bone!!

        // The amount we have moved in local space is where we are minus
        // where we were
        if (!logic_prev_translation_is_valid) {
          CalQuaternion logic_prev_rotation;
          (*iteratorCoreTrack)->getState(0.f, logic_prev_translation, logic_prev_rotation);
